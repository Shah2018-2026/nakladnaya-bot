...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "📊 Команда /report — статистика за день\n\n"
        "⚠️ Водитель должен написать на накладной:\n"
        "1️⃣ Фактический вес (например: 4,31)\n"
        "2️⃣ Код маршрута (например: А21, D15)"
    )
...
Никакого другого текста не добавляй."""
        ...
        # Отчёт в @Shah_forex_bot с хэштегами
        ...
        await send_to_forex(admin_msg, io.BytesIO(bytes(photo_bytes)))

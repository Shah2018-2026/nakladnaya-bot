...
Никакого другого текста не добавляй."""
        ...
        logger.info(f"Маршрут до нормализации: {маршрут_секция} | После: {marshut}")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если текст похож на код маршрута (буква + цифры) — ищем
    if re.match(r'^[А-ЯA-Zа-яa-z]{1,2}[-]?\d{1,3}$', text):
        marshut = normalize_marshut(text)
        rows = search_nakladnye(marshut=marshut)
        if not rows:
            await update.message.reply_text(
                f"📭 Накладных по маршруту *{marshut}* не найдено.",
                parse_mode="Markdown"
